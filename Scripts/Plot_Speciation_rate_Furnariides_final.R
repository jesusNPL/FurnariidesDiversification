...
pdf("paleo-envrionmental_variables_final.pdf")

#tiff(filename = "paleoenvironment_diversification.tif", width = 24, height = 24, units = "cm", 
#     pointsize = 12, compression = "lzw", bg = "white", res = 600)

#par(mfrow = c(2, 3), mar = c(4, 4, 0.2, 0.2))
par(mfrow = c(2, 3), mar = c(4, 4, 1, 1), oma = c(2, 2, 1, 1))

tree <- read.nexus("furna_divTime_WO_MCC.nex")
...
plot(-InfTemp[,"Age"], InfTemp[,"Temperature"], xlab = "", ylab = "Temperature (ºC)", 
     type = "p", lwd = 0.1, xlim = c(-crown.age, 0), ylim = c(0, max.y), col = "grey", las = 1, 
     cex = 0.8, cex.axis = 0.8, bty = "n")
temp.spl <- sm.spline(-InfTemp[,"Age"], InfTemp[,"Temperature"], df = 100)
lines(temp.spl, col = "firebrick1", lwd = 2)
legend("topleft", bty="n", c("a) Paleotemperature"), cex = 0.8)
# add vertical lines to delineate geological periods/epochs/stages
abline(v = c(-298.9, -252.2, -201.3, -145, -100.5, -66, -56, -47.8, -33.9, -28.1, -23.03, 
             -15.97, -11.62, -5.33, -2.58), col = "grey", lty = "dotted", lwd = "1") 
...
plot(-InfAlti[,"Age"], InfAlti[,"Altitude"], xlab = "", ylab = "Altitude (m)", type = "p", 
     lwd = 0.5, xlim = c(-crown.age, 0), col = "grey", las = 1, cex = 0.8, cex.axis = 0.8, 
     bty = "n")
alti.spl <- sm.spline(-InfAlti[,"Age"], InfAlti[,"Altitude"], df = 20)
lines(alti.spl, col = "burlywood4", lwd = 2)
legend("topleft", bty = "n", c("b) Andean paleoelevation"), cex = 0.8)
# add vertical lines to delineate geological periods/epochs/stages
abline(v = c(-66, -56, -47.8, -33.9, -28.1, -23.03, -15.97, -11.62, -5.33, -2.58), 
       col = "grey", lty = "dotted", lwd = "1") 
...
plot(-InfSea[,"Age"], InfSea[,"Level"], xlab = "", ylab = "Sea level (m)", type = "p", 
     lwd = 0.1, xlim = c(-crown.age,0), ylim = c(min.y, max.y), col = "grey", las = 1, 
     cex = 0.8, cex.axis = 0.8, bty = "n")
sea.spl <- sm.spline(-InfSea[,"Age"], InfSea[,"Level"], df = 100)
lines(sea.spl, col = "dodgerblue", lwd = 2)
legend("topleft", bty = "n", c("c) Past sea-level fluctuations"), cex = 0.8)
# add vertical lines to delineate geological periods/epochs/stages
abline(v = c(-298.9, -252.2, -201.3, -145, -100.5, -66, -56, -47.8, -33.9, -28.1, -23.03, 
             -15.97, -11.62, -5.33, -2.58), col = "grey", lty = "dotted", lwd = "1") 
...
attach("./Results/furna_divTime_500trees_complete_results_PastTemperature.Rdata")
...
# You may adjust the df value (the higher you put df the more accurate the curve will be)
res <- sm.spline(InfTemp[,"Age"], InfTemp[,"Temperature"], df = 100) 
Temp_fun <- function(x){predict(res, x)}

# Speciation rate
# Please replace the as.numeric() in the objects TempDep_lamb_par1 / 2 below with the 
# parameter estimates of your paleoenvironment-dependent analysis. 
...
plot(-InfTemp[,"Age"], f.lamb.mean(InfTemp[,"Age"]), ty = "l", col = "chartreuse3", 
     xlim = c(-crown.age, 0), ylim = c(0, max.y), lwd = 2, yaxt = "n", 
     xlab = "Time (Myrs ago)", ylab = "Diversification rates", 
     cex.axis = 0.8, bty = "n")
axis(2, at = seq(0, max.y, by = 0.05), las = 1, cex.axis = 0.8)
legend("topleft", bty = "n", c("d) Temperature-dependent"), cex = 0.8)
lines(-InfTemp[,"Age"], f.lamb.low(InfTemp[,"Age"]),ty = "l",col = "chartreuse3", 
      xlim = c(-crown.age,0), lwd = 1, lty = "dotted", yaxt = "n")
lines(-InfTemp[,"Age"], f.lamb.high(InfTemp[,"Age"]), ty = "l", col = "chartreuse3", 
      xlim = c(-crown.age, 0), lwd = 1, lty = "dotted", yaxt = "n")

# Extinction rate
# Please replace the as.numeric() in the objects TempDep_mu_par1 / 2 below with 
# the parameter estimates of your paleoenvironment-dependent analysis. 
...
lines(-InfTemp[,"Age"], f.mu.mean(InfTemp[,"Age"]), ty = "l", col = "white", lwd = 1, 
      yaxt = "n", xlab = "Time (Myrs ago)", 
      ylab = "Diversification rates", cex.axis = 0.8, bty = "n")
lines(-InfTemp[,"Age"], f.mu.low(InfTemp[,"Age"]), ty = "l", col = "white", lwd = 1, 
      lty = "dotted", yaxt = "n")
lines(-InfTemp[,"Age"], f.mu.high(InfTemp[,"Age"]), ty = "l", col = "white", lwd = 1, 
      lty = "dotted", yaxt = "n")
# add vertical lines to delineate geological periods/epochs/stages
abline(v = c(-298.9, -252.2, -201.3, -145, -100.5, -66, -56, -47.8, -33.9, -28.1, -23.03, 
             -15.97, -11.62, -5.33, -2.58), col = "grey", lty = "dotted", lwd = "1") 
...
attach("./Results/furna_divTime_500trees_complete_results_PastAndeanAltitude.Rdata")
mean.results <- final_table_tree_file[[1]][order(as.numeric(final_table_tree_file[[1]][,"AICc"])),]
std.err.results <- final_table_tree_file[[2]][order(as.numeric(final_table_tree_file[[2]][,"AICc"])),]
...
# Please replace the as.numeric() in the objects AltiDep_lamb_par1 / 2 below with 
# the parameter estimates of your paleoenvironment-dependent analysis. 
# TempDep_lamb_par1 is lambda, TempDep_lamb_par2 is alpha.
# TempDep_lamb_par1_sd is the standard error of lambda, and TempDep_lamb_par2_sd is the standard error of alpha. 
...
plot(-InfAlti[,"Age"], f.lamb.mean(InfAlti[,"Age"]), ty = "l", col = "chartreuse3", 
     xlim = c(-crown.age, 0), ylim = c(0, max.y), lwd = 2, yaxt = "n", 
     xlab = "Time (Myrs ago)", ylab = "Diversification rates", 
     cex.axis = 0.8, bty = "n")
axis(2, at = seq(0, max.y, by = 0.05), las = 1, cex.axis = 0.8)
legend("topleft", bty = "n", c("e) Altitude-dependent"), cex = 0.8)
lines(-InfAlti[,"Age"], f.lamb.low(InfAlti[,"Age"]), ty = "l", col = "chartreuse3", lwd = 1, 
      lty = "dotted", yaxt = "n")
lines(-InfAlti[,"Age"], f.lamb.high(InfAlti[,"Age"]), ty = "l", col = "chartreuse3", lwd = 1, 
      lty = "dotted", yaxt = "n")


# Extinction rate
# Please replace the as.numeric() in the objects AltiDep_mu_par1 / 2 below with 
#he parameter estimates of your paleoenvironment-dependent analysis. 
# TempDep_mu_par1 is mu, and TempDep_mu_par2 is beta.
# TempDep_mu_par1_sd is the standard error of mu, and TempDep_mu_par2_sd is the standard error of beta. 
...
lines(-InfAlti[,"Age"], f.mu.mean(InfAlti[,"Age"]), ty = "l", col = "red", 
      xlim = c(-crown.age,0), ylim = c(0, 0.8), lwd = 2, yaxt = "n", xlab = "Time (Myrs ago)", 
      ylab = "", cex.axis = 0.8, bty = "n")
lines(-InfAlti[,"Age"], f.mu.low(InfAlti[,"Age"]), ty = "l", col = "red", lwd = 1, 
      lty = "dotted", yaxt = "n")
lines(-InfAlti[,"Age"], f.mu.high(InfAlti[, "Age"]), ty = "l", col = "red", lwd = 1, 
      lty = "dotted", yaxt = "n")
# add vertical lines to delineate geological periods/epochs/stages
abline(v = c(-66, -56, -47.8, -33.9, -28.1, -23.03, -15.97, -11.62, -5.33, -2.58), 
       col = "grey", lty = "dotted", lwd = "1") 
...
attach("./Results/furna_divTime_500trees_complete_results_PastSeaLevel.Rdata")
mean.results <- final_table_tree_file[[1]][order(as.numeric(final_table_tree_file[[1]][,"AICc"])),]
std.err.results <- final_table_tree_file[[2]][order(as.numeric(final_table_tree_file[[2]][,"AICc"])),]
...
# Please replace the as.numeric() in the objects SeaDep_lamb_par1 / 2 below with 
#the parameter estimates of your paleoenvironment-dependent analysis. 
...
plot(-InfSea[,"Age"], f.lamb.mean(InfSea[,"Age"]), ty = "l", col = "chartreuse3", 
     xlim = c(-crown.age, 0), ylim = c(0, max.y), lwd = 2, yaxt = "n", 
     xlab = "Time (Myrs ago)", ylab = "Diversification rates", 
     cex.axis = 0.8, bty = "n")
axis(2, at = seq(0, max.y, by = 0.05), las = 1, cex.axis = 0.8)
legend("topleft", bty = "n", c("f) Sea level-dependent"), cex = 0.8)
lines(-InfSea[,"Age"], f.lamb.low(InfSea[,"Age"]),ty = "l", col = "chartreuse3", lwd = 1, 
      lty = "dotted", yaxt = "n")
lines(-InfSea[,"Age"], f.lamb.high(InfSea[,"Age"]), ty = "l", col = "chartreuse3", lwd = 1, 
      lty = "dotted", yaxt= "n")
...
lines(-InfSea[,"Age"], f.mu.mean(InfSea[,"Age"]), ty = "l", col = "red", 
      xlim = c(-crown.age,0), lwd = 2, yaxt = "n", xlab = "Time (Myrs ago)", ylab = "", 
      cex.axis = 0.8, bty = "n")
lines(-InfSea[,"Age"], f.mu.low(InfSea[,"Age"]), ty = "l", col = "red", lwd = 1, 
      lty = "dotted", yaxt = "n")
lines(-InfSea[,"Age"], f.mu.high(InfSea[,"Age"]), ty = "l", col = "red", lwd = 1, 
      lty = "dotted", yaxt = "n")
# add vertical lines to delineate geological periods/epochs/stages
abline(v = c(-66, -56, -47.8, -33.9, -28.1, -23.03, -15.97, -11.62, -5.33, -2.58), 
       col = "grey", lty = "dotted", lwd = "1") 
